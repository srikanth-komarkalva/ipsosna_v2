view: tbl_variables_values_v2 {
  derived_table: {
    sql: select distinct Brand_Fam as Value, "Brand_Fam" as VariableId, CategoryId FROM `mgcp-1192365-ipsos-gbht-srf617.YouTubeB2B.FactsV2` where Brand_Fam <> ' '
      ...
      union all
      select distinct Brand_MostEssential as Value, "Brand_MostEssential" as VariableId  ,CategoryId FROM `mgcp-1192365-ipsos-gbht-srf617.YouTubeB2B.FactsV2` where Brand_MostEssential <> ' '
      union all
